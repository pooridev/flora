...
const ProductType = memo(({ detail, onUpdatePrice }) => {
	...
	/**
	 * @description - This function is used to check if the product is allowed to be added to the basket
	 */
	const isAllowedToAdd = () => {
		// We can only have either Product or Subscription type in the basket
	...
	const addToCard = () => {
		...
		if (!isLoggedIn) {
			...
			if (existInBasketIndex !== -1) {
				...
				// basketData["total_price"] =
				// 	basketData["items"][existInBasketIndex]["quantity"] *
				// 	basketData["items"][existInBasketIndex]["price"];
			} else {
				isAllowedToAdd()
		...
		// check for quanity
		// const product = basketData["items"].find(
		// 	(item) => item["product_id"] === detail["id"]
		// );
		// if (product?.quantity >= stockQuantity) {
		// 	CustomAlert({
		// 		icon: "error",
		// 		text: "Sorry, we only have " + stockQuantity + " in stock",
		// 	});
		// 	return false;
		// }
	...
	return (
		<>
			<div className="ProductdetailInfo__orderrow">
				...
				>
					{stockQuantity !== 0 ? "ADD TO CART" : "Unavailable"}
				</button>
			</div>
			<div className="ProductdetailInfo_icontext">
				...
				<div className="product-attributes">
					{/* available attributes of product */}
					...
					{/*	{generalAttributes.box_color && (
							<div className="product-attribute">
							<h5 className="product-attribute__title">
								Box color
							</h5>
						 <div className="product-attribute__wrapper">
								{generalAttributes.box_color.map((elm) => (
									<div className="product-attribute__option">
										<input
											className="option-radio"
											name="box_color"
					...
					{/* {generalAttributes.flower_color && (
						<div className="product-attribute">
							<h5 className="product-attribute__title">
								Flower color
							</h5>
							<div className="product-attribute__wrapper">
								{generalAttributes.flower_color.map((elm) => (
									<div className="product-attribute__option">
										<input
											onChange={({ target }) =>
												setSelectedAttributes({
													flower_color: target.value,
												})
											}
											value={elm}
											className="option-radio"
					...
					{generalAttributes.flower_type && (
						<div className="product-attribute">
							<h5 className="product-attribute__title">
								Flower type
							</h5>
							<div className="product-attribute__wrapper">
								{generalAttributes.flower_type.map((elm) => (
									<div className="product-attribute__option">
										<input
											onChange={({ target }) =>
												setSelectedAttributes({
													flower_type: target.value,
												})
											}
											value={elm}
											className="option-radio"
